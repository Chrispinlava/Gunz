...
void onTick( CBlob@ this )
{
    ...
    else
    {
        ...
        {
            if (isTap( this ) || this.get_bool("release click"))
            {
				CBlob@[]@ closestBlobs;
				this.get("closest blobs", @closestBlobs );		
...
void GatherPickupBlobs( CBlob@ this )
{
    ...
    if (this.getMap().getBlobsInRadius( this.getPosition(), this.getRadius() + 250.0f, @blobsInRadius ))
    {
    ...
    if (this.get("pickup blobs", @pickupBlobs ))
    {
		...
		while (available.size() > 0)
        {
            ...
            for (uint i = 0; i < available.length; i++)
            {
				...
				f32 distToCursor = (bpos - aimpos).getLength();
				f32 dist = distToCursor * distToPlayer;
        ...
        // try to pick closest as facing - smaller than me

        //for (uint i = 0; i < closest.length; i++)
        //{
        //    CBlob @b = closest[i];
        //    const Vec2f bpos = b.getPosition();

        //    if ( ( facingLeft && bpos.x < pos.x ) || ( !facingLeft && bpos.x >= pos.x ) &&
        //            b.getRadius() < this.getRadius() )
        //    {
        //        server_Pickup( this, this, b );
        //        return;
        //    }
        //}

        // try to pick closest as facing
...
bool canBlobBePickedUp( CBlob@ this, CBlob@ blob )
{
    ...
    return ( ((pos2 - pos).getLength() < (this.getRadius() + blob.getRadius()) + 20.0f)
             && !blob.isAttached() 
             && !this.getMap().rayCastSolid( pos, pos2 )
           );
}
	...
	if (blob.isKeyPressed( key_pickup ))
	{
		...
		if (blob.get("pickup blobs", @pickupBlobs ))
		{
			...
			for (uint i = 0; i < pickupBlobs.length; i++)
			{
				...
				{
					hover = true;
					GUI::DrawText( b.getInventoryName(), b.getScreenPos() + Vec2f(-b.getWidth(), b.getHeight() + 16.0f), color_white );
